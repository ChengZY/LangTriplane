...
if __name__ == "__main__":
    bs = 4
    epoch = 100000
    plane_feature_dim = 10

    loss_fn = nn.MSELoss()
    class Tripplane(nn.Module):
        def __init__(self, plane_feature_dim):
            super(Tripplane, self).__init__()
            self.plane_feature = nn.Parameter(torch.rand((bs, 3, plane_feature_dim, 32, 32),dtype=torch.float32, device='cuda:0').requires_grad_(True))
    ...
    for i in range(epoch):
        points = torch.rand((bs, 100, 3),dtype=torch.float32, device='cuda:0').uniform_(0, 5)
